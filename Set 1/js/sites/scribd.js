$(document).ready(function()
{
		...
		else if(curPage(arrPages) == 1)
		{
			...
			smessage("red", "<br/>Click <i>JOIN NOW</i>");		
			
			$('html, body').animate({
				scrollTop: $('[name="word_user[name]"]').offset().top - 100
			},1000);
			
			vChange("name","word_user[name]", "username");
			vChange("name","word_user[password]", "password");
			vChange("name","email_address[email]", "useremail");
			vCustom("id","optin_login_lb",false);
			
			//fn_click("input", "value", "Join Now", 10);
		...
		else if(logged_in)
		{
			...
			{
				smessage("green", "<br/>Click <i>SAVE</i>, then you can move onto the next tab.");
			
				vChange("id","word_user_name","username");
				var bio = myvBio("name", "word_user[about]", true);
				$("#word_user_about").val(bio);
				vChange("id","word_user_interests","keywords");
				vChange("id","word_user_place","citystate");
				myVLinks("", "", "name","word_user[website]", false);
				vChange("name", "word_user[interests]", "keywords");
				$("a").each(function()
				{
					if($(this).html().indexOf("View Public Profile") != -1)
					{	
						var url = $(this).attr("href");
						var title = localStorage['firstname']+" "+localStorage['lastname']+" - Scribd";
			...
			{
				smessage("red", "<br/>Click <a href='https://www.scribd.com/account_settings' style='color:black !important;'><u>here</u></a> to continue.");	
